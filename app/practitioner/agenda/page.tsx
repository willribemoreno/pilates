...
export default function AgendaPage() {
  ...
  const openEditFromEvent = useCallback((evt: EventApi) => {
    ...
    const start = evt.start ?? (evt.startStr ? new Date(evt.startStr) : null);

    if (!start || isNaN(start.getTime())) {
      console.warn("[Agenda] Event has no valid start:", evt);
      return; // or show a toast and abort opening the modal
    }
  ...
  const handleDatesSet = useCallback(
    (_: DatesSetArg) => {
      queueMicrotask(() => {
        if (!calendarRef.current) return; // unmounted guard
        fetchEvents();
      });
    },
    [fetchEvents]
  ...
  const onEventClick = useCallback(
    (info: EventClickArg) => {
      ...
      if (isTouchLike) {
        ...
        queueMicrotask(() => openEditFromEvent(info.event));
        return;
      }
